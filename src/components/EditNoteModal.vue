...
const formIsValid = computed(() => {
    ...
    const isNotSameContent = (
      ...
      (removedLabels.value.length > 0 || addedLabels.value.length > 0)
    );
...
function filterLabel(labelToDelete) {
    const formattedAddedLabels = addedLabels.value.map(label => label.toLowerCase());
    const isDeletedLabelOnAddedLabels = formattedAddedLabels.includes(labelToDelete.toLowerCase());
    if (isDeletedLabelOnAddedLabels) {
      const newAddedLabels = addedLabels.value.filter(label => label.toLowerCase() != labelToDelete.toLowerCase());
...
function addLabel(labelItem) {
    const formattedDeletedLabels = removedLabels.value.map(label => label.toLowerCase());
    const isAddedLabelOnDeleteLabels = formattedDeletedLabels.includes(labelItem.toLowerCase());
    ...
    if (!isNewLabelExisting) {
      if (isAddedLabelOnDeleteLabels) {
        const newDeletedLabels = removedLabels.value.filter(label => label.toLowerCase() != labelItem.toLowerCase());
...
<template>
  <div class="modal" @click.self="closeEditModalFn">
    <div class="modal__card">
      ...
      <LabelForm :labels="noteLabels" @labelDeleted="filterLabel" @labelAdded="addLabel"/>
